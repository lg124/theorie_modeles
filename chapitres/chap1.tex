...
$\cL_{gp}$-structure même si ce n'est évidemment pas un groupe.
\end{exem}
...
\begin{proof}
Soit $\overline{a}=(a_1,\dots,a_n) \in M^n$ et $t$ un $\mathcal{L}$-terme, muni de variables libres dans $\overline{v} = (v_1,\dots,v_n)$.
Nous montrons par induction sur la complexité de $t$ que $j(t^{\mathcal{M}}(\overline{a}))
...
\item Une théorie $T$ est dite \kw{satisfaisable} s'il existe une $\cL$-structure $\cM$ tel que $\cM$ est un modèle de $T$.
\end{enumerate}
\end{defi}
...
\end{enumerate}
Cette théorie est satisfaisable: par exemple, $\mathbb{R}$ muni de la loi d'addition $+$ et du neutre $e_{\mathbb{R}} = 0$ en est un modèle.
\end{exem}
...
Soit $T$ une $\cL$-théorie. $T$ est satisfaisable si et seulement si tout sous-ensemble fini de $T$ est satisfaisable.
\end{theo*}
...
\begin{prop}
Soit $A \subset \mathcal{P}(I)$ non vide et ayant la propriété de l'intersection finie. L'ensemble des parties de $I$ qui contiennent une intersection finie 
d'éléments de $A$ est un filtre, qu'on appelle le filtre engendré par $A$.
\end{prop}

\begin{proof}
Soit un tel sous-ensemble $A \subset \mathcal{P}(I)$.\\ On montre que $ \cF = \{ X \subset I \mid \exists A_0,\dots,A_n \in A, A_0 \cap \dots \cap A_n \subset X \}$ est un filtre.
...
\begin{enumerate}[label= (\roman*)]
\item Si $I \setminus A \in \cF$, alors $A \cap (I \setminus A) = \emptyset$ et $\cF \cup \{A\}$ n'a pas la propriété d'intersection finie.
...
\begin{proof}
L'implication directe est claire. Soit $T$ une telle théorie. Pour chaque sous-ensemble fini $F$ de $T$, on appelle $\cM_F$ un modèle satisfaisant $F$.
...
$\phi$ étant une bijection entre structures sans symboles, c'est trivialement un $\cL$-isomorphisme.
\end{proof}

\noindent Le Théorème de compacité assure qu'une théorie reste satisfaisable tant que ses sous-ensembles finis le sont.
Pour une théorie $T$ finiment satisfaisable et à modèles infinis, on peut ajouter une infinité de symboles de constante et imposer qu'ils soient deux à deux distincts.
Ceci crée une nouvelle théorie, toujours finiment satisfaisable (toute partie finie de la nouvelle théorie ne demandent qu'un nombre fini de constantes, lesquelles peuvent 
...
\textbf{Fait :} $T \not\models \phi$ si et seulement si $T \cup \{\neg\phi\}$ est satisfaisable. En effet, si $T \models \phi$, alors $\neg\phi$ ne se satisfait dans aucun modèle de $T$,
donc $T \cup \{\neg\phi\}$ est insatisfaisable car un tel modèle devrait satisfaire à la fois $T$ et à la fois $\neg\phi$. Réciproquement, si $T \cup \{\neg\phi\}$ est insatisfaisable,
