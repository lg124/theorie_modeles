...
loi externe $K \times L \rightarrow L:(\lambda,x)\mapsto \lambda x$ est une
...
d'évaluation) $ev_a : K[X] \rightarrow K(a)$ tel que $\forall b \in K,
ev_a(b)=b$ et $ev_a(X) = a$.
\end{lemm}

\begin{defi}[Idéal annulateur]
Le noyau du morphisme d'évaluation $ev_b$ du lemme précédent est appelé
\kw{idéal anulteur} et noté $I(b)$, on a donc \[ I(b) := \{ f \in K[x] | f(b) =
0 \}. \]
Plus généralement, si $V\subset K$ est un ensemble (algébrique), l'\kw{idéal
annulateur} de $V$ est l'ensemble \[ I(b) := \{ f \in K[x] | f(b) =
0 \forall b \in V \}. \]
\end{defi}  
...
est le polynôme nul et $I(\pi) = \{0\}$ donc $ev_{\pi}: \bQ[X] \rightarrow
...
Tout corps $K$ admet une clôture algébrique, unique à isomorphisme près.
\end{theo}
...
\begin{prop}\label{prop:alg_clos_isom}
...
théorie des modèles. On va définir la théorie des cors algébriquement clos (ACF).
...
\begin{prop}[Elimination des quantificateurs]
